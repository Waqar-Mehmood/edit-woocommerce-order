...
function ewo_add_edit_order_button_after_order_table( $order ) {
    ...
    <p class="edit-order-container">
        <a href="#edit_order_popup_<?= $order->get_id() ?>" class="button edit-order"><?= __( 'Edit Order', 'woocommerce' ) ?></a>
    </p>
    ...
    if( ! empty( $edited ) ) {
        ...
        if( $order->has_status( 'cancelled' ) ) { 
            ...
            if( $order->get_payment_method() == 'cod' ||
                ...
                wc_print_notice( ewo_replace_string( esc_attr( get_option( 'ewo_popup_other_payment_methods' ) ), $order ), 'notice' ); 
            } else {
                wc_print_notice( ewo_replace_string( esc_attr( get_option( 'ewo_popup_paypal_payment_method' ) ), $order ), 'notice' ); 
            }    
        }
    }
}
